...
void get_init_params(int T, double *alpha, double *delta, double *sigv,
                     carray m, carray M, double *N, double *lambda, double *h)
{
    int           t;
    ...
    *sigv = sqrt(1.0 / rgamma(0.5 * (*N), 2.0 / (*lambda)));
    ...
    h = (double *)h0;
    for (t = 0; t < T; t++, h++) *h = exp(*h);

    PutRNGstate();
}

void update_params(int T, double *alpha, double *delta, double *sigv, carray m,
                   carray M, double *N, double *lambda, double *h)
{
    int     t;
...
void update_h(double *alpha, double *delta, double *sigv, double *h, double *r,
              int *T, double *h_new)
{
    int     t;
    double *h_minus, *h_plus;
    double mu, sigma2, phi, phi_LN, theta, theta_1, theta_LN, x_mode, c, p1, p2,
        q1, q2, num, denom;
    ...
    h_new[0] = exp((*alpha) + (*delta) * log(h_new[1]) + (*sigv) * norm_rand());
...
SEXP get_init_params_test(SEXP r)
{
    ...
    get_init_params(T, REAL(alpha), REAL(delta), REAL(sigv), m, M, REAL(N),
                    REAL(lambda), REAL(h));
...
SEXP update_params_test(SEXP h, SEXP m, SEXP M, SEXP lambda, SEXP N)
{
    ...
    *REAL(lambda_out) = *REAL(lambda);
    ...
    update_params(T, REAL(alpha), REAL(delta), REAL(sigv), m_out, M_out,
                  REAL(N_out), REAL(lambda_out), REAL(h));
