...
#define STATE_LEN 9
static void hUpdate1_chk(SEXP *currentState, int *numprot)
{
    int   i;
...
SEXP hUpdate1(SEXP currentState, SEXP maxit)
{
    int     numprot = 0;
    int     drawSuccessful;
    long    i, it, maxIter, T;
    double  alpha, delta, sigv, hcand, c, sigma, phi, mu, theta, xMode;
    double  p, q, pnew, qnew, fnew, f;
    double *hPtr, *rPtr;
    char   *names[STATE_LEN] = {"alpha", "delta",  "sigv", "m", "M",
                                "N",     "lambda", "h",    "r"};

    SEXP newState, hNew, r, h;

    hUpdate1_chk(&currentState, &numprot);

    alpha   = asReal(getListElt(currentState, "alpha"));
    delta   = asReal(getListElt(currentState, "delta"));
    sigv    = asReal(getListElt(currentState, "sigv"));
    maxIter = asInteger(maxit);
    PROT2(r = getListElt(currentState, "r"), numprot);
    PROT2(h = getListElt(currentState, "h"), numprot);
    T    = length(r);
    hPtr = REAL(h);
    rPtr = REAL(r);
    PROT2(hNew = NEW_NUMERIC(T), numprot);
    PROT2(newState = NEW_LIST(STATE_LEN), numprot);

    sigma = sqrt(sigv * sigv / (1 + delta * delta));
    phi   = (1 - 2 * exp(sigv * sigv)) / (1 - exp(sigv * sigv)) + 0.5;
    ...
    {
        mu =
            alpha * (1 - delta) + delta * (log(hPtr[i - 1]) + log(hPtr[i + 1]));
        mu    /= (1 + delta * delta);
        theta  = 0.5 * SQR(rPtr[i]) + (phi - 1) * exp(mu + 0.5 * SQR(sigma));
        xMode  = theta / (phi + 1);

        c  = 1.1 * R_pow(xMode, -1.5) * exp(-0.5 * SQR(rPtr[i]) / xMode);
        c *= exp(-0.5 * SQR(log(xMode) - mu) / SQR(sigma));
        c /= dgamma(1 / xMode, /* shape = */ phi + 2, /* scale = */ 1 / theta,
                    0);
        ...
        do
        {
            ...
            qnew  = dgamma(1 / hcand, /* shape = */ phi + 2,
                           /* scale = */ 1 / theta, 0);

            drawSuccessful  = unif_rand() < pnew / (c * qnew);
            it             += 1;

        } while (!drawSuccessful && it < maxIter);
        ...
        REAL(hNew)[i] = unif_rand() < (pnew / fnew) / (p / f) ? hcand : hPtr[i];
    }

    REAL(hNew)
    [0] = exp(alpha + delta * log(REAL(hNew)[1]) + sigv * norm_rand());
    REAL(hNew)
    [T - 1] = exp(alpha + delta * log(REAL(hNew)[T - 2]) + sigv * norm_rand());

    PutRNGstate();

    SET_ELT(newState, 0, ScalarReal(alpha));
    SET_ELT(newState, 1, ScalarReal(delta));
    SET_ELT(newState, 2, ScalarReal(sigv));
    SET_ELT(newState, 3, getListElt(currentState, "m"));
    SET_ELT(newState, 4, getListElt(currentState, "M"));
    SET_ELT(newState, 5, getListElt(currentState, "N"));
    SET_ELT(newState, 6, getListElt(currentState, "lambda"));
    SET_ELT(newState, 7, hNew);
    SET_ELT(newState, 8, r);

    set_names(newState, names);
...
static void updateParams0(long T, double *alpha, double *delta, double *sigv,
                          carray m, carray M, double *N, double *lambda,
                          double *h)
{
    long   t;
    int    ok;
    ...
    ok = is_matrix(m) && is_matrix(M) && 2 == NROW(m) && 1 == NCOL(m) &&
         2 == NROW(M) && 2 == NCOL(M);

    if (!ok) error("bad m or M");

    ok = alpha && delta && sigv && N && lambda && h;

    if (!ok) error("null pointer");
    ...
    {
        ARRAY2(Xt)[0][t - 1] = 1;
        ARRAY2(Xt)[1][t - 1] = log(h[t - 1]);
        ARRAY1(y)[t - 1]     = log(h[t]);
    }
    ...
    *lambda += sumsq(res) + *ARRAY1(tmp_1x1);

    /* Draw new sigv */
    *sigv = sqrt(1.0 / rgamma(*N / 2, 2.0 / lambda[0]));
...
#define STATE_LEN 9
SEXP updateParams1(SEXP currentState)
{
    int    numprot = 0;
    long   T;
    ...
    if (!isNewList(currentState) || STATE_LEN != length(currentState))
        error("currentState must be a list");
    ...
    delta  = asReal(getListElt(currentState, "delta"));
    sigv   = asReal(getListElt(currentState, "sigv"));
    m      = sexp_to_carray(getListElt(currentState, "m"), 1);
    M      = sexp_to_carray(getListElt(currentState, "M"), 1);
    N      = asReal(getListElt(currentState, "N"));
    lambda = asReal(getListElt(currentState, "lambda"));
    PROT2(h = getListElt(currentState, "h"), numprot);
    T = length(h);

    updateParams0(T, &alpha, &delta, &sigv, m, M, &N, &lambda, REAL(h));

    SET_ELT(newState, 0, ScalarReal(alpha));
    SET_ELT(newState, 1, ScalarReal(delta));
    SET_ELT(newState, 2, ScalarReal(sigv));
    SET_ELT(newState, 3, carray_to_sexp(m));
    SET_ELT(newState, 4, carray_to_sexp(M));
    SET_ELT(newState, 5, ScalarReal(N));
    SET_ELT(newState, 6, ScalarReal(lambda));
    SET_ELT(newState, 7, h);
    SET_ELT(newState, 8, getListElt(currentState, "r"));
...
#define ANSLEN 4
SEXP algo2(SEXP currentState, SEXP info)
{
    int     numprot = 0;
    int     output_h;
    long    i, n, initLoops, T;
    double *hPtr = NULL, *hOutPtr = NULL;
    SEXP    ans, alphaOut, deltaOut, sigvOut, hOut, maxit;
    char   *names[ANSLEN] = {"alpha", "delta", "sigv", "h"};

    n         = asInteger(getListElt(info, "numLoops"));
    initLoops = asInteger(getListElt(info, "initLoops"));
    output_h  = asInteger(getListElt(info, "output.h"));
    T         = length(getListElt(currentState, "r"));
    PROT2(maxit = getListElt(info, "maxit"), numprot);
    PROT2(alphaOut = NEW_NUMERIC(n), numprot);
    PROT2(deltaOut = NEW_NUMERIC(n), numprot);
    PROT2(sigvOut = NEW_NUMERIC(n), numprot);
    PROT2(hOut = output_h ? allocMatrix(REALSXP, T - 2, n) : R_NilValue,
          numprot);
    PROT2(ans = NEW_LIST(ANSLEN), numprot);

    for (i = 0; i < initLoops; i++)
    {
        PROTECT(currentState = updateParams1(currentState));
        PROTECT(currentState = hUpdate1(currentState, maxit));
    ...
    for (i = 0; i < n; i++)
    {
        ...
        REAL(alphaOut)[i] = asReal(getListElt(currentState, "alpha"));
        REAL(deltaOut)[i] = asReal(getListElt(currentState, "delta"));
        REAL(sigvOut)[i]  = asReal(getListElt(currentState, "sigv"));

        if (output_h)
        {
            hPtr    = REAL(getListElt(currentState, "h")) + 1;
            hOutPtr = matcol1(hOut, i);

            memcpy(hOutPtr, hPtr, (T - 2) * sizeof(double));
        }
...
static SEXP getInitState(SEXP r, SEXP initState, int diffusePrior)
{
    ...
    } else if (diffusePrior)
    {
        N = lambda = 0;
        sigv = delta = 0;
        alpha        = log(dvar(REAL(r) + 1, T - 2));
        m            = make_zero_matrix(2, 1);
        M            = make_zero_matrix(2, 2);
        ARRAY1(m)[0] = alpha;
        ARRAY1(M)[0] = ARRAY1(M)[3] = R_PosInf;

        for (t = 0; t < T; t++) hPtr[t] = exp(alpha);
    ...
    } else
    {
        ...
        N      = 2;
        lambda = 1 / 0.005;
        sigv = sqrt(1 / rgamma(/* shape = */ N / 2, /* scale = */ lambda / 2));
        ...
        for (t = 1; t < T; t++)
            loghPtr[t] = alpha + delta * loghPtr[t - 1] + sigv * norm_rand();
...
SEXP algo3(SEXP r, SEXP info, SEXP initState)
{
    ...
    double  alpha, delta, sigv, N, lambda, hcand, c, phi, mu, theta, qMode,
        sigma;
    double rho, sig1, sig2, e1, e2;
    double p, q, pnew, qnew, fnew, f;
        ...
        */
        ...
        for (i = 1; i < T - 1; i++)
        {
            ...
            if (drawSuccess)
            {
                ...
                hPtr[i] =
                    unif_rand() < (pnew / fnew) / (p / f) ? hcand : hPtr[i];
            }
        }
