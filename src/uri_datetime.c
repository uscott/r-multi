...
#ifdef Win32
    return TRUE;
#elif defined(__GLIBC__) && defined(__GLIBC_MINOR__) && __GLIBC__ >= 2 && \
    __GLIBC_MINOR__ >= 2
...
static double mktime00(struct tm *tm)
{
    ...
    if (year0 > 1970)
    {
        for (year = 1970; year < year0; year++) day += days_in_year(year);
    } else if (year0 < 1970)
    {
        for (year = 1969; year >= year0; year--) day -= days_in_year(year);
    }
...
static struct tm *localtime0(const double *tp, const int local, struct tm *ltm)
{
    ...
    /* month within year */
    for (mon  = 0; day >= (tmp = (days_in_month[mon]) +
                                ((mon == 1 && isleap(y)) ? 1 : 0));
         day -= tmp, mon++);
    res->tm_mon  = mon;
    res->tm_mday = day + 1;
...
static const char ltnames[][6] = {"sec",  "min",  "hour", "mday", "mon",
...
static void makelt(struct tm *tm, SEXP ans, int i, int valid)
{
    ...
    {
        for (j = 0; j < 8; j++) INTEGER(VECTOR_ELT(ans, j))[i] = NA_INTEGER;
...
SEXP do_asPOSIXlt(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    for (i = 0; i < 9; i++) SET_VECTOR_ELT(ans, i, allocVector(INTSXP, n));

    PROTECT(ansnames = allocVector(STRSXP, 9));
    for (i = 0; i < 9; i++) SET_STRING_ELT(ansnames, i, mkChar(ltnames[i]));
...
SEXP do_asPOSIXct(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    checkArity(op, args);
    x = CAR(args);
    if (!isVectorList(x) || LENGTH(x) != 9) error("invalid `x' argument");
    if (!isString((stz = CADR(args))) || LENGTH(stz) != 1)
        error("invalid `tz' value");
...
SEXP do_formatPOSIXlt(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (UseTZ == NA_LOGICAL) error("invalid `usetz' argument");
...
SEXP do_strptime(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (!isString((x = CAR(args)))) error("invalid `x' argument");
