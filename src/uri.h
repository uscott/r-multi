...
#define ENSURE_NUMERIC(x, i) \
    ( \
        (ensSxpPtr=&(x)) ? \
        (IS_NUMERIC(*ensSxpPtr) ? *ensSxpPtr : PROT2(*ensSxpPtr=AS_NUMERIC(*ensSxpPtr),i)): \
        R_NilValue \
    )
#define ENSURE_CHAR(x, i) \
    ( \
        (ensSxpPtr=&(x))? \
        (IS_CHARACTER(*ensSxpPtr)? *ensSxpPtr : PROT2(*ensSxpPtr=AS_CHAR(*ensSxpPtr), i)): \
        R_NilValue \
    )
#define ENSURE_INTEGER(x, i) \
    (\
        (ensSxpPtr=&(x))? \
        (IS_INTEGER(*ensSxpPtr)?*ensSxpPtr : PROT2(*ensSxpPtr=AS_INTEGER(*ensSxpPtr), i)): \
        R_NilValue \
    )
#define ENSURE_LENGTH(x, n, i) \
    (\
        (ensSxpPtr=&(x))? \
        (length(*ensSxpPtr)==(ensLenArg=(long)(n))?*ensSxpPtr: \
...
#define QUIT_IF(x, y, z) {if (x) {(y) = (z); return;}}
...
#define OPTION_PAYOFF(S,K,type) \
    (\
        ...
        ('c' == optionPayoffArg ? CALL_PAYOFF(S,K): \
            ('p' == optionPayoffArg ? PUT_PAYOFF(S,K) : \
                ('s' == optionPayoffArg ? STRD_PAYOFF(S,K) : \
                    NA_REAL))) \
    )


GLOBAL double stepFunArg;
#define STEP_FUN(x) ((stepFunArg = (double)(x)), (stepFunArg > 0.0 ? 1.0 : (0.0 == stepFunArg ? NA_REAL : 0.0)))
...
#define EXP_DELTA(S,K,type) \
    (\
        ...
        ('c' == expDeltaArg ? CALL_EXP_DELTA(S,K) : \
            ('p' == expDeltaArg ? PUT_EXP_DELTA(S,K) : \
                ('s' == expDeltaArg ? STRD_EXP_DELTA(S,K) : \
                    NA_REAL))) \
    )
...
double near(double refval, long place, double *refptr,
            double *outptr, long ptrlen);
...
void get_random_sample(const double *in, double *out, const long inlen, long outlen);
void get_random_sample2(const double *src, double *out, const long srclen, long outlen);
...
#define uritm_greaterOrEqual(a,b) (uritm_greaterThan(a,b) || uritm_equals(a,b))
