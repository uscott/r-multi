...
#define INT2VEC(n) \
    (((n) > 0) ? (((n) * sizeof(int) - 1) / sizeof(VECREC) + 1) : 0)
#define FLOAT2VEC(n) \
    (((n) > 0) ? (((n) * sizeof(double) - 1) / sizeof(VECREC) + 1) : 0)
#define COMPLEX2VEC(n) \
    (((n) > 0) ? (((n) * sizeof(Rcomplex) - 1) / sizeof(VECREC) + 1) : 0)
#define PTR2VEC(n) \
    (((n) > 0) ? (((n) * sizeof(SEXP) - 1) / sizeof(VECREC) + 1) : 0)
#else
...
R_stdGen_ptr_t R_set_standardGeneric_ptr(
    R_stdGen_ptr_t newValue); /* set method */
