...
SEXP dSubMatrix(SEXP x, long row1, long row2, long col1, long col2)
{
    ...
    ok = isMatrix(x) && 0 <= row1 && row1 <= row2 && row2 < mx && 0 <= col1 &&
         col1 <= col2 && col2 < nx;

    if (!ok) error("bad args dSubMatrix");
...
void dnormalize(double *x, long len)
{
    ...
    for (i = 0, y = x; i < len; i++, y++) norm += SQR(*y);

    norm = sqrt(norm);

    for (i = 0; i < len; i++) *x++ /= norm;
}
...
SEXP numrep(SEXP x, const long n)
{
    ...
    for (j = 1; j < n; j++)
        for (i = 0; i < m; i++) REAL(x)[i + m * j] = REAL(x)[i];
...
SEXP intrep(SEXP x, const long n)
{
    ...
    for (j = 1; j < n; j++)
        for (i = 0; i < m; i++) INTEGER(x)[i + m * j] = INTEGER(x)[i];
...
SEXP charrep(SEXP x, const long n)
{
    ...
    for (j = 1; j < n; j++)
        for (i = 0; i < m; i++) CHAR_PTR(x)[i + m * j] = CHAR_PTR(x)[i];
...
SEXP listrep(SEXP x, const long n)
{
    ...
    for (j = 1; j < n; j++)
        for (i = 0; i < m; i++) SET_ELT(x, i + m * j, GET_ELT(x, i));
...
void cvect_to_string(SEXP src, char *dest)
{
    ...
    if (!IS_CHARACTER(src)) error("!IS_CHARACTER(src)");

    for (i = 0; i < n; i++) *dest++ = *CHAR(GET_ELT(src, i));
}
...
void set_names(SEXP dest, char **names)
{
    ...
    for (i = 0; i < n; i++) CHAR_PTR(nameS)[i] = mkChar(*names++);
 ...
 *******************************************************************/
{
    ...
    for (j = 0, p = REAL(mat) + i; j < n; j++, p += m) *ans++ = *p;
}
...

{
    ...
    for (j = 0, p = REAL(mat) + i; j < n; j++, p += m, ans++) *ans = p;
}
 ...
 *******************************************************************/
{
    ...
    for (j = 0; j < n; j++) *ans++ = *CHAR(STRING_ELT(mat, m * j));
}
...

{
    ...
    for (j = 0, p = INTEGER(mat) + i; j < n; j++, p += m, ans++) *ans = p;
}
...
SEXP getListElt(SEXP list, const char *str)
{
    ...
    if (!isNewList(list)) error("bad args getListElt");
...

{
    ...
    if (n <= 0) error("n <= 0 passed to int_max");

    while (++x < end) ans = ans < *x ? *x : ans;
...

{
    ...
    if (len < 0) error("negative length passed to lmax");
...

{
    ...
    if (n <= 0) error("n <= 0 passed to double_max");

    while (++x < xn) ans = ans < *x ? *x : ans;
...
double double_min(const double *x, const int n)
{
    ...
    if (n <= 0) error("n <= 0 passed to double_min");
...

{
    ...
    for (t = 0; t < new_order; t++) *ans++ = NA_REAL;
    ...
    for (t = new_order; t < (*n); t++)
    {
        ...
        *ans = *yPtr;
        ...
        for (s = new_order; s > 0; s--) *ans += *(--yPtr);
...

{
    ...
    if (n > xlen - 1 || n < 0) error("bad inputs to C function which_index");

    memcpy(xCopy, x, xlen * sizeof(double));

    for (i = 0; i < xlen; i++) ind[i] = i;
   ...
   xlen is the length of [x]. */
{
    ...
    if (n > xlen - 1 || n < 0) error("bad inputs to C function which1");
   ...
   long n is the length of x */
{
    ...
    for (i = 0; i < xlen; i++) dist[i] = ABS(x[i] - y);
...
double near(double refval, long n, double *refptr, double *ans, long ptr_length)
...
void set_seed_uri(const unsigned long seed) { idum_uriel_scott = seed; }
...
void get_random_sample(const double *src, double *out, const long srclen,
                       long outlen)
...

{
    ...
    while (outlen-- > 0) *out++ = src[get_rand_uri(srclen)];
}
 ...
 *******************************************************************/
{
    ...
    SEXP gradient = R_NilValue, R_fcall, par_new;

    if (!isFunction(fn)) error("Bad function input in feval");
    if (!isEnvironment(env)) error("Bad environment input in feval");
    if (!isNumeric(par)) error("Bad parameters input in feval");
