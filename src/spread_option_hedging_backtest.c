...
static int sprdOptDh1_chk(SEXP *T, SEXP *S1, SEXP *S2, SEXP *K, SEXP *vol1,
                          SEXP *vol2, SEXP *rho, SEXP *r, SEXP *q1, SEXP *q2,
                          SEXP *opt, SEXP *nGrdPts, int *numprot)
{
#define NNARGS 10
    int   i, ok, allFinite;
    long  t, T_len, S1_len, S2_len;
    SEXP *args[NNARGS + 1] = {T, S1, S2, K, vol1, vol2, rho, r, q1, q2, opt};

    for (i = 0; i < NNARGS; i++)
        if (0 == args[i]) error("Null pointer.");

    for (i = 0; i < NNARGS; i++) ENSURE_NUMERIC(*args[i], *numprot);
    ...
    S1_len = length(*S1);
    S2_len = length(*S2);
    if (T_len != S1_len - 1 || S1_len != S2_len) error("Length problems");

    for (i = 3; i < NNARGS; i++)
        if (1 == length(*args[i]))
            PROT2(*args[i] = numrep(*args[i], T_len), *numprot);

    if (1 == length(*opt)) PROT2(*opt = charrep(*opt, T_len), *numprot);

    ok = T_len == length(*opt);
    for (i = 3; i < NNARGS; i++) ok *= T_len == length(*args[i]);

    if (!ok) error("More length problems");
...
SEXP sprdOptDh1(SEXP T, SEXP S1, SEXP S2, SEXP K, SEXP vol1, SEXP vol2,
                SEXP rho, SEXP r, SEXP q1, SEXP q2, SEXP opt, SEXP nGrdPts)
{
    int    numprot = 0;
    int    allFinite;
    long   i, T_len;
    ...
    allFinite = sprdOptDh1_chk(&T, &S1, &S2, &K, &vol1, &vol2, &rho, &r, &q1,
                               &q2, &opt, &nGrdPts, &numprot);
    ...
    {
        pearson0(REAL(T)[i] / 365, REAL(S1)[i], REAL(S2)[i], REAL(K)[i],
                 REAL(vol1)[i], REAL(vol2)[i], REAL(rho)[i], REAL(r)[i],
                 REAL(q1)[i], REAL(q2)[i], *CHAR(GET_ELT(opt, i)),
                 asInteger(nGrdPts), /* calcDeltas = */ 1, tmp);
        ...
        if (dt <= 0) error("Can't have negative time increments");
...
SEXP listSprdOptDh1(SEXP tau, SEXP S1, SEXP S2, SEXP K, SEXP vol1, SEXP vol2,
                    SEXP rho, SEXP r, SEXP q1, SEXP q2, SEXP opt, SEXP nGrdPts)
{
    int  numprot = 0;
    long i, N;
    ...
    listSprdOptDh1_chk(&tau, &S1, &S2, &K, &vol1, &vol2, &rho, &r, &q1, &q2,
                       &opt, &nGrdPts, &numprot);
...
static void sprdOptDh2_chk(SEXP *T, SEXP *S1, SEXP *S2, SEXP *K, SEXP *vols1,
                           SEXP *vols2, SEXP *volStrikes1, SEXP *volStrikes2,
                           SEXP *rho, SEXP *r, SEXP *q1, SEXP *q2, SEXP *opt,
                           SEXP *nGrdPts, int *numprot)
{
#define N_VEC_ARGS 8
#define N_MAT_ARGS 4
    ...
    for (i = 0; i < N_VEC_ARGS; i++) ENSURE_NUMERIC(*vecArgs[i], *numprot);
    ...
    if (length(*S1) != length(*S2)) error("Unequal price series lengths");
    ...
    for (i = 0; i < N_MAT_ARGS; i++)
        if (nrows(*matArgs[i]) != T_len) error("not enough rows");

    if (ncols(*vols1) != ncols(*volStrikes1)) error("column mismatch");

    if (ncols(*vols2) != ncols(*volStrikes2)) error("column mismatch");

    if (1 == length(*opt)) PROT2(*opt = charrep(*opt, T_len), *numprot);

    ok = (T_len == length(*opt));

    for (i = 4; i < N_VEC_ARGS && ok; i++) ok *= (T_len == length(*vecArgs[i]));

    for (i = 0; i < N_MAT_ARGS && ok; i++) ok *= (T_len == nrows(*matArgs[i]));
    ...
    if (!allFinite) error("ooh");
#undef N_VEC_ARGS
#undef N_MAT_ARGS
}

static void getDeltas(SEXP T, SEXP S1, SEXP S2, double K, SEXP vols1,
                      SEXP vols2, SEXP rho, SEXP volStrikes1, SEXP volStrikes2,
                      SEXP r, SEXP q1, SEXP q2, char optionType, long nGrdPts,
                      SEXP d1, SEXP d2, double *sig1, double *sig2, double *K1,
                      double *K2)
{
    ...
    for (i = 0; i < T_len; i++)
    {
        ...
        pearson0(REAL(T)[i] / 365.0, REAL(S1)[i], REAL(S2)[i], K, vol1, vol2,
                 REAL(rho)[i], REAL(r)[i], REAL(q1)[i], REAL(q2)[i], optionType,
                 nGrdPts,
    ...
    REAL(d1)
    [T_len] = EXP_DELTA(REAL(S1)[T_len], REAL(S2)[T_len] - K, optionType);
    REAL(d2)
    [T_len] = EXP_DELTA(REAL(S2)[T_len], REAL(S1)[T_len] + K, optionType);
}
                           ...
                           SEXP undPosn2, int period)
{
    ...
    for (i = 0; i < S_len; i++)
    {
        ...
        dayCtr +=
            i < T_len - 2 ? REAL(T)[i] - REAL(T)[i + 1] : REAL(T)[T_len - 1];
    }
}

static void getHedgePLandTC(SEXP T, SEXP S1, SEXP S2, SEXP r, SEXP q1, SEXP q2,
                            SEXP undPosn1, SEXP undPosn2, double tcUnd1,
                            double tcUnd2, int isRelUndTc1, int isRelUndTc2,
                            double *deltaHedgePL, double *totalTC)
{
    long   i, T_len = length(T), S_len = T_len + 1;
    double und1TCtoday, und2TCtoday, dt;
    double ir = asReal(r), div1 = asReal(q1), div2 = asReal(q2), PLtoday = 0,
           discFac = 1;

    if (!deltaHedgePL || !totalTC) error("null pointers");
    ...
    *totalTC = und1TCtoday + und2TCtoday;
    ...
    {
        dt       = i < T_len ? (REAL(T)[i - 1] - REAL(T)[i]) / 365.0
                             : REAL(T)[T_len - 1] / 365.0;
        discFac *= exp(-ir * dt);
...
SEXP sprdOptDh2(SEXP T, SEXP S1, SEXP S2, SEXP K, SEXP vols1, SEXP vols2,
                SEXP volStrikes1, SEXP volStrikes2, SEXP rho, SEXP r, SEXP q1,
                SEXP q2, SEXP opt, SEXP nGrdPts)
{
    ...
    sprdOptDh2_chk(&T, &S1, &S2, &K, &vols1, &vols2, &volStrikes1, &volStrikes2,
                   &rho, &r, &q1, &q2, &opt, &nGrdPts, &numprot);
    ...
    getDeltas(T, S1, S2, asReal(K), vols1, vols2, rho, volStrikes1, volStrikes2,
              r, q1, q2, optionType, asInteger(nGrdPts), d1, d2, sig1, sig2, K1,
              K2);

    getDeltaHedges(T, d1, d2, undPosn1, undPosn2, /* period = */ 1);

    getHedgePLandTC(T, S1, S2, r, q1, q2, undPosn1, undPosn2,
                    /* tcUnd1 */ 0, /* tcUnd2 */ 0,
                    /* isRelUndTc1 */ 0, /* isRelUndTc2 */ 0, &deltaHedgePL,
                    &totalTC);
...
static void listSprdOptDh2_chk(SEXP *tau, SEXP *S1, SEXP *S2, SEXP *K,
                               SEXP *vols1, SEXP *vols2, SEXP *volStrikes1,
                               SEXP *volStrikes2, SEXP *rho, SEXP *r, SEXP *q1,
                               SEXP *q2, SEXP *opt, SEXP *nGrdPts, int *numprot)
{
#define NUM_ARGS 13
    long  i, N = 0;
    SEXP *args[NUM_ARGS] = {tau,         S1,  S2, K,  vols1, vols2, volStrikes1,
                            volStrikes2, rho, r,  q1, q2,    opt};
...
SEXP listSprdOptDh2(SEXP tau, SEXP S1, SEXP S2, SEXP K, SEXP vols1, SEXP vols2,
                    SEXP volStrikes1, SEXP volStrikes2, SEXP rho, SEXP r,
                    SEXP q1, SEXP q2, SEXP opt, SEXP nGrdPts)
{
    int  numprot = 0;
    long i, N;
    ...
    listSprdOptDh2_chk(&tau, &S1, &S2, &K, &vols1, &vols2, &volStrikes1,
                       &volStrikes2, &rho, &r, &q1, &q2, &opt, &nGrdPts,
                       &numprot);
...
static int sprdOptDh3_chk(SEXP *T, SEXP *S1, SEXP *S2, SEXP *K, SEXP *vol1,
                          SEXP *vol2, SEXP *rho, SEXP *r, SEXP *q1, SEXP *q2,
                          SEXP *opt, SEXP *nGrdPts, int *numprot)
{
#define NNARGS 10
    ...
    ok = 1;
    for (i = 4; i < NNARGS; i++) ok *= T_len == length(*args[i]);
...
SEXP sprdOptDh3(SEXP T, SEXP S1, SEXP S2, SEXP K, SEXP vol1, SEXP vol2,
                SEXP rho, SEXP r, SEXP q1, SEXP q2, SEXP opt, SEXP nGrdPts,
                SEXP maxdt)
{
    enum
    ...
    char       option_type;
    char      *rlist_names[ANS_LEN] = {
        "hedge", "option", "o1", "o2", "o3", "h1",   "h2",   "h3", "d1",
        "d2",    "tau",    "S1", "S2", "K",  "vol1", "vol2", "rho"};
    ...
    sprdOptDh3_chk(&T, &S1, &S2, &K, &vol1, &vol2, &rho, &r, &q1, &q2, &opt,
                   &nGrdPts, &numprot);
    ...
    for (i = 0; i < T_len; i++)
    {
        pearson0(REAL(T)[i] / 365, REAL(S1)[i], REAL(S2)[i], X, REAL(vol1)[i],
                 REAL(vol2)[i], REAL(rho)[i], REAL(r)[i], REAL(q1)[i],
                 REAL(q2)[i], option_type, grdp, /* calcDeltas = */ 1, tmp);
        ...
        dt[i] =
            i < T_len - 1 ? REAL(T)[i] - REAL(T)[i + 1] : REAL(T)[T_len - 1];
        dt[i] /= 365;

        if (dt[i] <= 0) error("Unexpected non-positive time increment.");

        discFac *= exp(-REAL(r)[i] * dt[i]);

        pltoday = -del1 * (REAL(S1)[i + 1] - REAL(S1)[i]);
        pltoday -=
            del1 * REAL(S1)[i] * expm1((REAL(q1)[i] - REAL(r)[i]) * dt[i]);
        pltoday -= del2 * (REAL(S2)[i + 1] - REAL(S2)[i]);
        pltoday -=
            del2 * REAL(S2)[i] * expm1((REAL(q2)[i] - REAL(r)[i]) * dt[i]);
    ...
    for (i = 1; i < T_len + 1; i++)
    {
        if (dt[i - 1] > max_dt) REAL(h1)
        [i] = REAL(h2)[i] = REAL(h3)[i] = REAL(o1)[i] = REAL(o2)[i] =
            REAL(o3)[i]                               = NA_REAL;
        else
        {
            ...
            REAL(o1)
            [i] = den > 0 ? (REAL(dval)[i] - REAL(dval)[i - 1]) / den : NA_REAL;
...
SEXP listSprdOptDh3(SEXP tau, SEXP S1, SEXP S2, SEXP K, SEXP vol1, SEXP vol2,
                    SEXP rho, SEXP r, SEXP q1, SEXP q2, SEXP opt, SEXP nGrdPts,
                    SEXP maxdt)
{
    int  numprot = 0;
    ...
    listSprdOptDh3_chk(&tau, &S1, &S2, &K, &vol1, &vol2, &rho, &r, &q1, &q2,
                       &opt, &nGrdPts, &numprot);
...
static int sprdOptDh4_chk(SEXP *T, SEXP *S1, SEXP *S2, SEXP *K, SEXP *vol1,
                          SEXP *vol2, SEXP *rho, SEXP *shortRate,
                          SEXP *longRate, SEXP *q1, SEXP *q2, SEXP *opt,
                          int *numprot)
{
#define NNARGS 11
    int   i, ok, allFinite;
...
SEXP sprdOptDh4(SEXP T, SEXP S1, SEXP S2, SEXP K, SEXP vol1, SEXP vol2,
                SEXP rho, SEXP shortRate, SEXP longRate, SEXP q1, SEXP q2,
                SEXP opt, SEXP nGrdPts, SEXP maxdt)
{
    enum
    {
        i_hedge,
    ...
    int       numprot = 0;
    long      i, T_len;
    double    optnProfit, del1, del2, pltoday, discFac, den, *dt,
        tmp[PEARSON0_PTR_LEN], X;
    double totalHedgesProfit = 0, totalOptionProfit = 0;
    char   option_type;
    char  *rlist_names[ANS_LEN] = {"hedgeProfits",
                                   "optionValues",
                                   "o1",
                                   "o2",
                                   "o3",
    ...
    SEXP rlist;
    SEXP hedgesProfit1, optionProfit1, hedgesProfit2, optionProfit2,
        optionProfit3, hedgesProfit3;
    SEXP d1, d2, names;
    ...
    sprdOptDh4_chk(&T, &S1, &S2, &K, &vol1, &vol2, &rho, &shortRate, &longRate,
                   &q1, &q2, &opt, &numprot);
    ...
    {
        pearson0(REAL(T)[i] / 365, REAL(S1)[i], REAL(S2)[i], X, REAL(vol1)[i],
                 REAL(vol2)[i], REAL(rho)[i], /*r*/ REAL(longRate)[i],
                 REAL(q1)[i], REAL(q2)[i], option_type, grdp,
                 /* calcDeltas */ 1, tmp);

        REAL(sprdOptionVal)[i] = discFac * tmp[PEARSON0_PRICE_INDEX];

        if (0 == i) totalOptionProfit = -tmp[PEARSON0_PRICE_INDEX];
    ...
    totalOptionProfit += exp(-REAL(longRate)[0] * REAL(T)[0] / 365.0) *
                         tmp[PEARSON0_PRICE_INDEX];
    ...
    for (i = 1; i < T_len + 1; i++)
    {
        ...
        {
            REAL(hedgesProfit1)
            [i] = REAL(hedgesProfit2)[i] = REAL(hedgesProfit3)[i] = NA_REAL;
            REAL(optionProfit1)
            [i] = REAL(optionProfit2)[i] = REAL(optionProfit3)[i] = NA_REAL;
        ...
        } else
        {
            ...
            REAL(hedgesProfit1)
            [i] = den > 0 ? REAL(dailyProfitHedges)[i] / den : NA_REAL;
            REAL(hedgesProfit2)
            [i] = den > 0 ? REAL(hedgesProfit1)[i] / sqrt(dt[i - 1]) : NA_REAL;
            REAL(optionProfit1)[i] = den > 0 ? optnProfit / den : NA_REAL;
            REAL(optionProfit2)
            [i] = den > 0 ? REAL(optionProfit1)[i] / sqrt(dt[i - 1]) : NA_REAL;
...
static void listSprdOptDh4_chk(SEXP *tau, SEXP *S1, SEXP *S2, SEXP *K,
                               ...
                               SEXP *shortRate, SEXP *longRate, SEXP *q1,
                               SEXP *q2, SEXP *opt, int *numprot)
{
#define NARGS 12
    long  i, N = 0;
    SEXP *args[NARGS] = {tau, S1,        S2,       K,  vol1, vol2,
                         rho, shortRate, longRate, q1, q2,   opt};
...
SEXP listSprdOptDh4(SEXP tau, SEXP S1, SEXP S2, SEXP K, SEXP vol1, SEXP vol2,
                    SEXP rho, SEXP shortRate, SEXP longRate, SEXP q1, SEXP q2,
                    SEXP opt, SEXP nGrdPts, SEXP maxdt)
{
    int  numprot = 0;
    ...
    listSprdOptDh4_chk(&tau, &S1, &S2, &K, &vol1, &vol2, &rho, &shortRate,
                       &longRate, &q1, &q2, &opt, &numprot);
    ...
    for (i = 0; i < N; i++)
    {
        tmp = sprdOptDh4(GET_ELT(tau, i), GET_ELT(S1, i), GET_ELT(S2, i),
                         GET_ELT(K, i), GET_ELT(vol1, i), GET_ELT(vol2, i),
                         GET_ELT(rho, i), GET_ELT(shortRate, i),
                         GET_ELT(longRate, i), GET_ELT(q1, i), GET_ELT(q2, i),
                         GET_ELT(opt, i), nGrdPts, maxdt);
