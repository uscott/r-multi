                      ...
                      double *lower, double *upper)
{
  ...
  if (!upper || !lower)
    error ("null pointer where not allowed in getBounds");
  ...
  *upper = *lower = impcorr;
  ...
  if (err > 0) { /* initial correlation too low */
  ...
  } else if (err < 0) { /* initial correlation too high */

    do {

      it += 1;
      *lower += lower[0] < -1 ? runif(0,lower[0]) : -(width *= runif(1,2))* unif_rand();
...
static void impCorr_chkargs(SEXP *tau, SEXP *S1, SEXP *S2,
                            SEXP *K, SEXP *vol1, SEXP *vol2,
                            SEXP *mprice, SEXP *r, SEXP *q1,
...
#define N_NUMARGS 10
  ...
  SEXP *args[N_NUMARGS+2] = 
    {tau, S1, S2, K, 
     vol1, vol2, mprice, r, 
     q1, q2, op_type, initGuess};

  char *names[N_NUMARGS+2] = 
    {"tau", "S1", "S2", "K", 
     "vol1", "vol2", "mprice", "r", 
     "q1", "q2", "op_type", "initGuess"};
  
  if (grdPts < 0)
      error ("Negative number of grid pts.");
...
SEXP impCorr(SEXP tau, SEXP S1, SEXP S2, SEXP K, 
             SEXP vol1, SEXP vol2, SEXP mprice, SEXP r, 
