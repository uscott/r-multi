...
/**********************************************************************/
/*
**  Functions for computing implied volatility.
*/
/**********************************************************************/
/**********************************************************************/
/**********************************************************************/
...
static double bserr(const double T, const double S, const double K,
                    const double vol, const double r, const double q,
                    const char opt, const double mprice)
{
...
static void impvol0(const double T, const double S, const double K,
                    ...
                    const char optype, const double tol, int maxit, double *iv)
...
{
    int    it = 0, ok;
    double lower, upper, err, err_l, err_u, errA, errB, bnd_lower = 0.0,
                                                        bnd_upper = 0.0;

    if (!iv) error("Null pointer passed where not allowed");
    ...
    switch (optype)
    {
        ...
        case 'p':
            bnd_lower = MAX(exp(-r * T) * K - exp(-q * T) * S, 0);
            bnd_upper = K;
            break;
...
static void imp_vol_chkargs(SEXP *tau, SEXP *S, SEXP *K, SEXP *mprice, SEXP *r,
                            SEXP *q, SEXP *op_type, int *numprot)
{
    int i, N = 0;
...
SEXP imp_vol(SEXP tau, SEXP S, SEXP K, SEXP mprice, SEXP r, SEXP q,
...

{
    ...
    for (i = 0; i < N; ++i)
    {
        impvol0(REAL(tau)[i] / 365.0, REAL(S)[i], REAL(K)[i], REAL(mprice)[i],
                REAL(r)[i], REAL(q)[i], *CHAR(STRING_ELT(op_type, i)),
