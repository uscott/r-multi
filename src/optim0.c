...
#define NUM_TRIES 2
static int NumDeriv(OptimFunc f, double *par, long parlen, void *context,
                    double *gradient, double *eps, double *par_new)
 ...
 *******************************************************************/
{
    ...
    for (i = 0; i < len; ++i)
    {
        if (0.0 == ABS(eps[i]) || !R_FINITE(eps[i])) eps[i] = sqrt(DOUBLE_EPS);

        if (unif_rand() < 0.5) eps[i] *= -1.0;
...
SEXP OptimGradient00(OptimFunc f, double *initPar, long parlen, void *context,
 ...
 *******************************************************************/
{
    ...
    for (i = 0; i < parlen; ++i) eps[i] = sqrt(DOUBLE_EPS);

    GetRNGstate();

    for (it = 0, convergence = 1; it < maxit && convergence; ++it)
    {
        if (!NumDeriv(f, *p1, parlen, context, gradient, eps, *p2)) continue;
                     ...
                     const long maxit)
{
    int   numprot = 0;
    long  j, m, n;
    char *ans_names[OPTIM_ANS_LEN] = OPTIM_ANS_NAMES;
    ...
    if (IS_MATRIX(initpar))
    {
        ...
        for (j = 0; j < n; ++j)
        {
            ...
            tmp2 = OptimGradient00(f, tmp1, m, context, tol, relTol, minimize,
                                   maxit);
    ...
    } else if (isNewList(initpar))
    {
        ...
        for (j = 0; j < n; ++j)
        {
            ...
            tmp1 = (double *)malloc(m * sizeof(double));
            memcpy(tmp1, REAL(GET_ELT(initpar, j)), m * sizeof(double));
    ...
    {
        ans = OptimGradient00(f, REAL(initpar), parlen, context, tol, relTol,
                              minimize, maxit);
        PROT2(ans, numprot);
    }
...
static void OptimGa0_check(SEXP *initpar, const int parlen,
                           const int basePopSize, int *numprot)
{
    ...
    if (isNull(*initpar) || !length(*initpar))
    {
        ...
        for (i = 0; i < parlen; ++i) REAL(*initpar)[i] = 1e-4;
...
static void GetNextGen(OptimFunc f, long parlen, void *context, SEXP pop,
 ...
 *******************************************************************/
{
    ...
    {
        GetNextGen(f, parlen, context, pop, fargs, fvals, ix, tmp, basePopSize,
                   minimize);
                 ...
                 SEXP relTol, SEXP optimType)
{
    ...
    switch (type)
    {
        case 0:
            ...
            ans = OptimGa0(testOptimFun, initPar, controlPar, asInteger(parlen),
        ...
        case 1:
            ...
            ans = OptimGradient0(testOptimFun, initPar, controlPar, asReal(tol),
