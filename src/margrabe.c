               ...
               double *ans)
{
    ...
    if (!ans) error("Illegal null pointer passed");

    for (i = 0; i < MARGRABE0_PTR_LEN; i++) ans[i] = NA_REAL;

    ok = T >= 0 && S1 >= 0 && S2 >= 0 && R_FINITE(q1) && vol1 >= 0 &&
         vol2 >= 0 && R_FINITE(T) && R_FINITE(S1) && R_FINITE(S2) &&
         R_FINITE(vol1) && R_FINITE(r) && R_FINITE(vol2) && R_FINITE(q2) &&
         R_FINITE(rho);
    ...
    switch (opt)
    {
        case 'c':
            ...
            if (!EXPIRATION)
            {
                ...
                d1  = (log(S2 / S1) + (q1 - q2 + 0.5 * SQR(vol)) * T) /
                     (vol * sqrt(T));
                d2   = d1 - vol * sqrt(T);
                Nd1  = NORM_DIST(d1);
                Nd2  = NORM_DIST(d2);
                ...
                v = S2 * sqrt(T) * Npd1 * div2 /
                    vol; /* Used in vega1, vega2 and eta */
            }

            ans[MARGRABE0_PRICE_INDEX] = EXPIRATION
                                           ? DMAX(S2 - S1, 0)
                                           : div2 * S2 * Nd1 - div1 * S1 * Nd2;
            ans[MARGRABE0_D1_INDEX] =
                EXPIRATION ? STEP_FUN(S2 - S1) : -div1 * Nd2;
            ans[MARGRABE0_D2_INDEX] =
                EXPIRATION ? -STEP_FUN(S1 - S2) : div2 * Nd1;
            ans[MARGRABE0_G1_INDEX]  = EXPIRATION
                                         ? (S1 == S2 ? NA_REAL : 0)
                                         : div1 * Npd2 / (S1 * vol * sqrt(T));
            ans[MARGRABE0_G2_INDEX]  = EXPIRATION
                                         ? (S1 == S2 ? NA_REAL : 0)
                                         : div2 * Npd1 / (S2 * vol * sqrt(T));
        ...
        case 's':
            ...
            for (i = 0; i < MARGRABE0_PTR_LEN; i++) ans[i] = c[i] + p[i];
...
#define N_NUMARGS 9
    ...
    char *names[N_NUMARGS + 1] = {"tau", "S1", "S2", "vol1", "vol2",
                                  "rho", "r",  "q1", "q2",   "opt"};
    /* Coerce types  */
    ...
    /* Find maximal length */
    N = 0;
    for (i = 0; i < N_NUMARGS + 1; i++) N = MAX(N, length(*args[i]));
              ...
              SEXP r, SEXP q1, SEXP q2, SEXP opt)
{
    ...
    margrabe_chkargs(&tau, &S1, &S2, &vol1, &vol2, &rho, &r, &q1, &q2, &opt,
                     &numprot);
    ...
    for (i = 0; i < N; i++)
        margrabe0(REAL(tau)[i] / 365.0, REAL(S1)[i], REAL(S2)[i], REAL(vol1)[i],
                  REAL(vol2)[i], REAL(rho)[i], REAL(r)[i], REAL(q1)[i],
                  REAL(q2)[i], *CHAR(GET_ELT(opt, i)), ARRAY2(ans_matrix)[i]);
                        ...
                        double *e1, double *e2, double *ans)
{
    ...
    if (!ans || !e1 || !e2) error("Illegal null pointer passed");
...
static void margrabe2_chkargs(SEXP *tau, SEXP *S1, SEXP *S2, SEXP *K,
                              SEXP *vol1, SEXP *vol2, SEXP *rho, SEXP *r,
                              SEXP *q1, SEXP *q2, SEXP *opt, SEXP *npaths,
                              SEXP *eps, int *numprot)
{
#define N_NUMARGS 10
    int   i, N = 0, n;
    ...
    if (isNull(*eps) || !IS_MATRIX(*eps) || 2 != ncols(*eps) ||
        0 == nrows(*eps))
    {
        n    = (int)ceil(sqrt(asInteger(*npaths))) + 1;
        *eps = PROTECT(
            bvt_normuri(ScalarInteger(n), ScalarReal(0.0), ScalarInteger(1)));
        *numprot += 1;
    }
               ...
               SEXP eps)
{
    ...
    margrabe2_chkargs(&tau, &S1, &S2, &K, &vol1, &vol2, &rho, &r, &q1, &q2,
                      &opt, &npaths, &eps, &numprot);
    ...
    for (i = 0; i < N; i++)
        margrabe2_0(REAL(tau)[i] / 365.0, REAL(S1)[i], REAL(S2)[i], REAL(K)[i],
                    REAL(vol1)[i], REAL(vol2)[i], REAL(rho)[i], REAL(r)[i],
                    REAL(q1)[i], REAL(q2)[i], *CHAR(GET_ELT(opt, i)),
                    nrows(eps), e1, e2, ARRAY2(ans_matrix)[i]);
