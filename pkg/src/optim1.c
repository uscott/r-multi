...
#define NUM_TRIES 2
static int numDeriv2(optimFun f, SEXP x, SEXP controlPar, SEXP gradient,
                     SEXP eps, SEXP xNew)
...

{
    ...
    double    *xPtr = REAL(x), *xNewPtr = REAL(xNew), *gradPtr = REAL(gradient),
           *epsPtr = REAL(eps);

    ok = IS_NUMERIC(eps) && IS_NUMERIC(gradient) && IS_NUMERIC(xNew) &&
         IS_NUMERIC(x) && (xLen == length(gradient)) &&
         (xLen == length(xNew)) && (xLen == length(eps));
...
SEXP optimGradient1(optimFun f, SEXP initPar, SEXP controlPar, const double tol,
 ...
 *******************************************************************/
{
    ...
    SEXP ans = R_NilValue, par1 = R_NilValue, par2 = R_NilValue,
         gradient = R_NilValue, eps = R_NilValue, *x = &par1, *xNew = &par2,
         *tmp = NULL;
    ...
    for (i = 0; i < parLen; i++) REAL(eps)[i] = sqrt(DOUBLE_EPS);

    GetRNGstate();

    for (it = 0, convergence = 1; it < maxit && convergence; it++)
    {
        if (!numDeriv2(f, *x, controlPar, gradient, eps, *xNew)) continue;
                    ...
                    const int relTol, const int minimize, const long maxit)
{
    ...
    if (IS_MATRIX(initPar))
    {
        ...
        for (j = 0; j < n; j++)
        {
            ...
            tmp2 = optimGradient1(f, tmp1, controlPar, tol, relTol, minimize,
                                  maxit);
            PROTECT(tmp2);
    ...
    {
        ans = optimGradient1(f, initPar, controlPar, tol, relTol, minimize,
                             maxit);
        PROT2(ans, numprot);
    }
...
static void optimGa2_chk(SEXP *initPar, const int parLen, const int basePopSize,
                         int *numprot)
{
    long i       = 0;
    long initLen = 0;
...
static void getNextGen(optimFun f, SEXP controlPar, SEXP pop, SEXP fargs,
                       SEXP fvals, SEXP ix, SEXP tmp, const int basePopSize,
                       const int minimize)
{
    const int    bigPopSize = ncols(pop);
    const int    parLen     = nrows(pop);
...

{
    ...
    {
        getNextGen(f, controlPar, pop, fargs, fvals, ix, tmp, basePopSize,
                   minimize);
...
SEXP testOptim(SEXP initPar, SEXP controlPar, SEXP parLen, SEXP basePopSize,
               SEXP stopLags, SEXP minit, SEXP maxit, SEXP minimize, SEXP tol,
               SEXP relTol, SEXP optimType)
{
    ...
    switch (type)
    {
        case 0:
            ...
            ans = optimGa2(testOptimFun, initPar, controlPar, asInteger(parLen),
                           asInteger(basePopSize), asInteger(stopLags),
                           asInteger(minit), asInteger(maxit),
                           asInteger(minimize), asReal(tol), asInteger(relTol));
            break;

        case 1:

            ans = optimGradient2(testOptimFun, initPar, controlPar, asReal(tol),
                                 asInteger(relTol), asInteger(minimize),
                                 asInteger(maxit));
