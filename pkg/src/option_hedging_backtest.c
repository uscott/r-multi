...

{
    ...
    if (!ok) error("null pointer passed to bsd1");

    ok = R_FINITE((double)n) && R_FINITE(K) && R_FINITE((double)hedgePeriod);

    if (!ok) warning("Improper n, K or hedgePeriod");

    ok = R_FINITE(r) && R_FINITE(q);

    if (!ok) warning("Improper r or q");

    ok = n > 0 || *T >= 0 || K >= 0 || *vol >= 0;

    if (!ok) warning("n <= 0 or *T < 0 or K < 0 or *vol < 0");

    ok = ('c' == optionType || 'p' == optionType || 's' == optionType);

    if (!ok) warning("optionType not one of c, p, s");
}

static void bsd1(const int Tlen, const double *T, const double *S,
                 const double K, const double *vol, const double r,
                 const double q, const int hedgePeriod, const char optionType,
                 double *del)
...

{
    ...
    {
        ok = R_FINITE(T[i]) && R_FINITE(S[i]) && T[i] >= 0 && vol[i] >= 0 &&
             T[i - 1] > T[i];
...
static int bsdh1(const long Tlen, const int numStrikes, const double *T,
                 const double *S, const double *K, const double *vol,
                 const double *posn, const double r, const double q,
                 const int hedgePeriod, char *optionTypes[],
                 const double *transCosts, const int reltc, double *deltas,
                 double *PL, double *dly_PL, double *totalTransCosts)
...

{
    ...
    ok = Tlen > 0 && T && S && K && vol && posn && numStrikes > 0 &&
         optionTypes && deltas && PL && transCosts && totalTransCosts && dly_PL;

    if (!ok) error("null pointer passed or negative lengths");
    ...
    {
        bsd1(Tlen, T, S, *K++, vol, r, q, hedgePeriod, *optionTypes[j],
             tmpDeltas);

        for (i = 0; i <= Tlen; i++) deltas[i] += posn[j] * tmpDeltas[i];
    }
...
#define NARGS 11
static int bsDeltaHedge1_chk(SEXP *T, SEXP *S, SEXP *K, SEXP *vol, SEXP *posn,
                             SEXP *r, SEXP *q, SEXP *hedgePeriod,
                             SEXP *optionTypes, SEXP *transCosts, SEXP *reltc,
                             int *numprot)
{
    long T_len, S_len;
    int  i, numStrikes;

    SEXP *args[NARGS] = {T, S,           K,           vol,        posn, r,
                         q, hedgePeriod, optionTypes, transCosts, reltc};
    ...
    if (1 == length(*vol)) PROT2(*vol = numrep(*vol, T_len), *numprot);

    if (1 < length(*r)) warning("only 1st value of r used");

    if (1 < length(*q)) warning("only 1st value of q used");

    if (1 == length(*transCosts))
        PROT2(*transCosts = numrep(*transCosts, S_len), *numprot);

    if (T_len != length(*vol)) error("vol wrong length");

    if (S_len != length(*transCosts)) error("transCosts wrong length");
                   ...
                   SEXP reltc)
{
    ...
    bsDeltaHedge1_chk(&T, &S, &K, &vol, &posn, &r, &q, &hedgePeriod,
                      &optionTypes, &transCosts, &reltc, &numprot);

    T_len      = length(T);
    S_len      = length(S);
    numStrikes = length(K);

    if (numStrikes > MAX_NUM_OPTNS) error("too many strikes!");
    ...
    bsdh1(T_len, numStrikes, REAL(T), REAL(S), REAL(K), REAL(vol), REAL(posn),
          *REAL(r), *REAL(q), hPeriod, opt, REAL(transCosts), asInteger(reltc),
          REAL(deltas), &PL, REAL(dly_PL), &totalTransCosts);
...
#define NARGS 12
static void bsDeltaHedge2_chk(SEXP *tau, SEXP *S, SEXP *K, SEXP *vol,
                              SEXP *posn, SEXP *r, SEXP *q, SEXP *hedgePeriod,
                              SEXP *optionTypes, SEXP *transCosts, SEXP *reltc,
                              SEXP *returnDaily, int *numprot)
{
    long  S_len_m1;
    int   numStrikes, i, j;
    SEXP  tmp;
    ...
    if (S_len_m1 <= 0) error("need at least 2 price observations");
    ...
    if (!IS_MATRIX(*K)) error("K is not a matrix");

    numStrikes = ncols(*K);

    if (numStrikes <= 0) error("need positive number of strikes");

    if (1 == length(*r)) PROT2(*r = numrep(*r, S_len_m1), *numprot);

    if (1 == length(*vol)) PROT2(*vol = numrep(*vol, S_len_m1), *numprot);

    if (1 == length(*q)) PROT2(*q = numrep(*q, S_len_m1), *numprot);
    ...
    if (numStrikes != length(*posn)) error("numStrikes != length(posn)");
    ...
    if (numStrikes > MAX_NUM_OPTNS) error("numStrikes > MAX_NUM_OPTNS");
}
#undef NARGS
...

{
    ...
    bsDeltaHedge2_chk(&tau, &S, &K, &vol, &posn, &r, &q, &hedgePeriod,
                      &optionTypes, &transCosts, &reltc, &returnDaily,
                      &numprot);
    ...
    for (i = 0; i < S_len_m1; i++)
    {
        ...
        bsdh1(m, numStrikes, REAL(tau) + i, REAL(S) + i, tmpK, REAL(vol) + i,
              REAL(posn), REAL(r)[i], REAL(q)[i], hPeriod, opt,
              REAL(transCosts) + i, relTc, tmpDeltas, REAL(PL) + i, tmpDlyPL,
              REAL(totalTransCosts) + i);

        if (retDly)
        {
            memcpy(REAL(GET_ELT(deltas, i)), tmpDeltas,
                   (m + 1) * sizeof(double));
            memcpy(REAL(GET_ELT(dly_PL, i)), tmpDlyPL,
                   (m + 1) * sizeof(double));
        }
    }
...
#define NARGS 12
static void bsDeltaHedge3_chk(SEXP *tau, SEXP *S, SEXP *K, SEXP *vol,
                              SEXP *posn, SEXP *r, SEXP *q, SEXP *hedgePeriod,
                              SEXP *optionTypes, SEXP *transCosts, SEXP *reltc,
                              SEXP *returnDaily, int *numprot)

{
    ...
    for (i = 0; i < NARGS; i++) PROT2(*args[i] = AS_LIST(*args[i]), *numprot);
    ...
    if (1 != listLen)
        for (i = 1; i < NARGS; i++)
            if (1 == length(*args[i]))
            {
                ...
                warning("Replacing %s with rep(%s, %d)", names[i], names[i],
                        listLen);
            }
...

{
    ...
    bsDeltaHedge3_chk(&tau, &S, &K, &vol, &posn, &r, &q, &hedgePeriod,
                      &optionTypes, &transCosts, &reltc, &returnDaily,
                      &numprot);
    ...
    for (i = 0; i < listLen; i++)
    {
        tmp = bsDeltaHedge2(GET_ELT(tau, i), GET_ELT(S, i), GET_ELT(K, i),
                            GET_ELT(vol, i), GET_ELT(posn, i), GET_ELT(r, i),
                            GET_ELT(q, i), GET_ELT(hedgePeriod, i),
                            GET_ELT(optionTypes, i), GET_ELT(transCosts, i),
                            GET_ELT(reltc, i), GET_ELT(returnDaily, i));
...
static void dvHedge_chk(SEXP *posn, SEXP *posnStrikes, SEXP *optionTypes,
                        SEXP *T, SEXP *S, SEXP *vols, SEXP *volStrikes, SEXP *r,
                        SEXP *q, SEXP *hedgePeriod, SEXP *undTransCosts,
                        ...
                        SEXP *isRelOptCosts, int *numprot)
{
    ...
    const SEXPTYPE type[] = {REALSXP, REALSXP, STRSXP,  REALSXP,
                             REALSXP, REALSXP, REALSXP, REALSXP,
    ...
    if (S_len != T_len + 1) error("T must be 1 less than S in length");

    if (S_len < 2) error("Price series must have length >= 2");

    if (!isMatrix(*vols) || !isMatrix(*volStrikes))
        error("vols and volStrikes must be matrices");

    ok = (nrows(*vols) == nrows(*volStrikes) &&
          ncols(*vols) == ncols(*volStrikes));

    if (!ok) error("Non-conforming vol and volStrike matrices");
...
static void getPosnDeltasVegas(SEXP posn, SEXP posnStrikes, SEXP optionTypes,
                               SEXP T, SEXP S, SEXP vols, SEXP volStrikes,
                               SEXP r, SEXP q, SEXP atmVol, SEXP atmStrike,
                               SEXP deltas, SEXP vegas, double *sigma,
                               double *K)
...

{
    ...
    for (i = 0; i < S_len; i++)
    {
        ...
        {
            bs0(tau, REAL(S)[i], REAL(posnStrikes)[j], vol, *REAL(r), *REAL(q),
...
static void getHedges(SEXP T, SEXP S, SEXP vols, SEXP volStrikes, SEXP r,
                      SEXP q, SEXP atmVol, SEXP atmStrike, SEXP deltas,
                      SEXP vegas, SEXP strdVol, SEXP prevStrdVol,
                      SEXP strdPrice, SEXP prevStrdPrice, SEXP strdPosn,
                      SEXP strdStrike, SEXP undPosn, double *sig, double *K,
                      int period)
...

{
    ...
    for (i = 0; i < T_len; i++)
    {
        ...
        if (dayCtr >= period || !i)
        {
            ...
            REAL(undPosn)
            [i] = -(REAL(deltas)[i] + REAL(strdPosn)[i] * bsOut[1]);
        ...
        if (i)
        {
            ...
            bs0(tau, REAL(S)[i], REAL(strdStrike)[i - 1], REAL(prevStrdVol)[i],
                *REAL(r), *REAL(q), 's', bsOut);
    ...
    bs0(0, REAL(S)[S_len - 1], REAL(strdStrike)[S_len - 2], 0.01, 0, 0, 's',
        bsOut);
...
static void getHedgePLandTC(SEXP T, SEXP S, SEXP r, SEXP q, SEXP strdPrice,
                            SEXP prevStrdPrice, SEXP strdPosn, SEXP strdStrike,
                            SEXP undPosn, double tcUnd, double tcOpt,
...

{
    ...
    char *names[ANS_LEN] = {
        "strdPosn",  "undPosn",       "strdVol", "prevStrdVol", "strdStrike",
        "strdPrice", "prevStrdPrice", "deltas",  "vegas",       "undPL",
        "strdPL",    "undTC",         "strdTC"};

    SEXP strdVol, prevStrdVol, strdStrike, strdPrice, prevStrdPrice, atmVol,
        atmStrike, deltas, vegas, strdPosn, undPosn, ans;

    dvHedge_chk(&posn, &posnStrikes, &optionTypes, &T, &S, &vols, &volStrikes,
                &r, &q, &hedgePeriod, &undTransCosts, &optTransCosts,
    ...
    getPosnDeltasVegas(posn, posnStrikes, optionTypes, T, S, vols, volStrikes,
                       r, q, atmVol, atmStrike, deltas, vegas, sig, K);

    getHedges(T, S, vols, volStrikes, r, q, atmVol, atmStrike, deltas, vegas,
              strdVol, prevStrdVol, strdPrice, prevStrdPrice, strdPosn,
              strdStrike, undPosn, sig, K, period);

    getHedgePLandTC(T, S, r, q, strdPrice, prevStrdPrice, strdPosn, strdStrike,
                    undPosn, tcUnd, tcOpt, isRelUndTc, isRelOptTc,
                    &deltaHedgePL, &vegaHedgePL, &totalUndTC, &totalOptTC);
...
static void dvHedge2_chk(SEXP *posn, SEXP *posnStrikes, SEXP *optionTypes,
                         ...
                         SEXP *isRelUndCosts, SEXP *isRelOptCosts, int *numprot)
{
    ...
    ok = isMatrix(*posnStrikes) && isMatrix(*vols) && isMatrix(*volStrikes) &&
         nrows(*posnStrikes) == T_len && nrows(*volStrikes) == T_len &&
         nrows(*vols) == T_len && ncols(*posnStrikes) == length(*posn) &&
         length(*posn) == length(*optionTypes);

    if (!ok) error("bleh bleh");

    if (1 == length(*r)) PROT2(*r = numrep(*r, T_len), *numprot);
    if (1 == length(*q)) PROT2(*q = numrep(*q, T_len), *numprot);

    ok = length(*r) == T_len && length(*q) == T_len;

    if (!ok) error("bleh bleh bleh");
