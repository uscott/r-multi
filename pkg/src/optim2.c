...
SEXP optim_gradient1(const SEXP fn, const SEXP init, const SEXP tol,
                     const SEXP minimize, const SEXP maxit, const SEXP env)
...

{
    ...
    ok = isFunction(fn) && isEnvironment(env) && isInteger(maxit) &&
         isNumeric(init);

    if (!ok) error("error in optim_gradient1");
    ...
    if (R_finite(*REAL(feval(fn, init, env))))
    {
        ...
        for (it = 0, convergence = 1; it < mit && convergence; it++)
        {
            ...
            *vp1 = *REAL(feval(fn, par1, env));
            ...
            for (i = 0; i < n; i++)
                *pp2++ =
                    *pp1++ - sgn * (*gp++) * MAX(1.0, fabs(*vp1)) * stepsize;
    ...
    {
        for (i = 0; i < length(ans); i++) SET_ELT(ans, i, ScalarReal(NA_REAL));
    }
...

{
    ...
    ok = isMatrix(par) && isMatrix(ans) && 0 < nrows(par) &&
         nrows(par) == nrows(ans) && ncols(ans) == ncols(par) + C2(ncols(par));

    if (!ok) error("bad arguments in breed1");
...

{
    ...
    ok = isMatrix(par) && isMatrix(mut) && ncols(par) > 0 &&
         ncols(mut) == 2 * ncols(par) && MAX(nrows(par), nrows(mut)) > 0 &&
         nrows(par) == nrows(mut);

    if (!ok) error("bad argument passed to mutate1");
...
SEXP optim_ga1(SEXP fn, SEXP init, SEXP maxit, SEXP minimize, SEXP tol,
               SEXP env)
{
    ...
    if (!ok) error("optim_ga1: bad args");
    ...
    for (gen = 0, convergence = 1; gen < mg && convergence; gen++)
    {
        ...
        for (j = 0; j < len[0]; j++)
            memcpy(matcol1(pop[0], j), matcol1(pop[2], ix[j]),
                   m * sizeof(double));
    ...
    for (i = 0; i < n; i++) REAL(fvals)[i] *= sgn;
