...
static int ng11_aug(const int    *T,
                    const double *x,
                    const double *y,
                    const double *m0,
                    const double *lambda, 
                    ...
                    double       *ll)
{
  ...
  ok = 
    T  && x  && m0    && lambda && a00 &&  
    b1 && a1 && gamma && ll     && a01 && y;

  if (!ok)
    error ("null pointer passed where not allowed");

  ok = 
    R_FINITE(*T)   && R_FINITE(*m0)    && R_FINITE(*lambda) &&
    R_FINITE(*a00) && R_FINITE(*b1)    && 
    R_FINITE(*a1)  && R_FINITE(*gamma) && R_FINITE(*a01);
  ...
  if (uncond) {
    ...
    if (h[0] <= 0)
      h[0] = sum_product(x, x, *T) / *T - dmean(x, *T);
...
#define NPAR 8
static void ngarch11_aug_chkargs(SEXP *x, 
                                 SEXP *y, 
                                 SEXP *par,
                                 int  *numprot)
{
...
static void sim_aug_ng11_path(const long    T,
                              const double *y,
                              const double *par,
                              const double *x0,
                              const double *h0,                              
                              ...
                              double       *h)
{
  ...
  if (!ok)
    error ("sim_aug_ng11_path: Null pointer passed where not allowed");

  ok = R_FINITE(*x0) && R_FINITE(*h0);

  if (!ok)
    error ("sim_aug_ng11_path: Improper x0 or h0");
...
#define NPAR  7
#define NARGS 7
static void sim_aug_ng11_chkargs(SEXP *T,
                                 SEXP *npaths,
                                 SEXP *y,
                                 SEXP *par,
                                 SEXP *x0,
                                 ...
                                 int  *numprot)
{
  ...
  if (1 == length(*y))
    PROT2(*y = numrep(*y, asInteger(*T)), *numprot);

  if (asInteger(*T) != length(*y))
    error ("y has wrong length");
                  ...
                  SEXP z)
{
  ...
  char *names[2 + NARGS] = {"x", "h", "T", "npaths", "y", "par",
                            "x0", "h0", "z"};
...
#define NPAR 9
static int ng11_decay(const int    *xLen,
                      const double *x,
                      const double *dt,
                      const double *y,
                      const double *age,
...
static void ngarch11_decay_chkargs(SEXP *x,
                                   SEXP *dt,
                                   SEXP *y,
                                   SEXP *age,
                                   SEXP *par,
...
static void sim_decay_ng11_path(const long    pathLen,
                                const double *dt,
                                const double *y,
                                const double *age,
                                const double *par,
                                ...
                                double       *h)
{
  ...
  if (!ok)
    error ("sim_ng11: Null pointer passed where not allowed");
...
#define NARGS  7
static void sim_decay_ng11_chkargs(SEXP *pathLen,
                                   SEXP *numPaths,
                                   SEXP *dt,
                                   SEXP *y,
                                   SEXP *age,
                                   ...
                                   int  *numprot)
{
  ...
  char *names[NARGS] = {"pathLen", "numPaths", "dt", 
                        "y", "age", "par", "z"};
                    ...
                    SEXP z)
{
  ...
  char *names[2 + NARGS] = {"x", "h", "T", "numPaths", 
                            "dt", "y", "age", "par", "z"};
...
static int ng11_seasonal(const int    *T,
                         const double *x,
                         const double *yday,
                         const double *m0,
                         const double *lambda, 
                         ...
                         double       *ll)
{
  ...
  ok = 
    T  && x  && m0    && lambda && a00 && omega && 
    b1 && a1 && gamma && ll     && a01 && yday;
  ...
  ok = 
    R_FINITE(*T)   && R_FINITE(*m0)    && R_FINITE(*lambda) &&
    R_FINITE(*a00) && R_FINITE(*b1)    && R_FINITE(*omega)  &&
    R_FINITE(*a1)  && R_FINITE(*gamma) && R_FINITE(*a01);
...
#define N_SEXP_ARGS 3
#define NPAR 9
static void ngarch11_seasonal_chkargs(SEXP *x, 
                                      SEXP *yday, 
                                      SEXP *par,
                                      int  *numprot)
{
  ...
  if (length(*x) != length(*yday))
    error ("length(x) != length(yday)");
...

{
  ...
  ok = 
    R_FINITE(*T)   && R_FINITE(*m0)   && R_FINITE(*lambda) &&
    R_FINITE(*phi) && R_FINITE(*a0)   && R_FINITE(*b1)     &&
    R_FINITE(*a1)  && R_FINITE(*gamma);
  ...
  if (ok) {
    ...
    *ll = *T * log(M_2PI) + log(*h) + SQR(*z);
    ...
    for (0; x < xT && R_FINITE(*ll); x++, Lx++, h += inc, Lh += inc) {
      ...
      *z   = (*x - *m0 - *phi * (*Lx)) / sqrt(*h) - *lambda + 0.5 * sqrt(*h);
      *ll += log(*h) + SQR(*z);
...
static SEXP ar1ng11_matrix_apply(const SEXP x, 
                                 const double *m0,
                                 const double *lambda, 
                                 const double *phi,
                                 const double *a0, 
                                 ...
                                 const double *xmin1)
{
  ...
  double llsum = 0.0;
  char  *names[LEN] = {"x", "m0", "lambda", "phi", 
                       "a0", "a1",
                       "b1", "gamma", "h0", "xmin1", "h", 
                       "res", "ll", "ll.sum"};
  ...
  if (!isMatrix(x))
    error ("C error: ng11_matrix_apply.  Non-matrix passed.");

  ok = 
    m0 && lambda && phi && a0 && a1 && b1 && gamma && h0 && xmin1;

  if (!ok)
    error ("Null pointer passed to ar1ng11_matrix_apply");
...
static SEXP ar1ng11_list_apply(const SEXP x, 
                               const double *m0,
                               const double *lambda, 
                               const double *phi,
                               const double *a0, 
                               ...
                               const double *xmin1)
{
  ...
  if (!isNewList(x))
    error ("C error: ng11_lst.  Non-list passed.");

  ok = 
    m0 && lambda && phi && a0 && a1 && b1 && gamma && h0 && xmin1;
  
  if (!ok)
    error ("Null pointer passed to ar1ng11_list_apply");
...
static void ar1ng11_apply_chkargs(SEXP *x, 
                                  SEXP *par, 
                                  SEXP *fn,
                                  SEXP *arity, 
                                  SEXP *env,
                                  int  *numprot)
{
  ...
  ok = 
    (isMatrix(*x) || isNewList(*x))    &&
    (length(par) == asInteger(*arity)) &&
    isFunction(fn) && isEnvironment(env);

  if (!ok)
    error ("ar1ng11_apply.  Wrong argument types.");
...

{
  ...
  if (isMatrix(x)) {
    ...
    PROTECT(ans = ar1ng11_matrix_apply(x, 
                                       f + 0 * n, f + 1 * n, 
                                       f + 2 * n, f + 3 * n,
                                       f + 4 * n, f + 5 * n,
                                       f + 6 * n, f + 7 * n,
    ...
    PROTECT(ans = ar1ng11_list_apply(x, 
                                     f + 0 * n, f + 1 * n, 
                                     f + 2 * n, f + 3 * n,
                                     f + 4 * n, f + 5 * n,
                                     f + 6 * n, f + 7 * n,
...
static void sim_ar1ng11_path(const long    T,
                             const double *m0,
                             const double *lambda,
                             const double *phi,
                             const double *a0, 
...
#define NARGS 12
static void sim_ar1ng11_chkargs(SEXP *T,
                                SEXP *npaths,
                                SEXP *m0,
                                SEXP *lambda,
                                SEXP *phi,
                                ...
                                int  *numprot)
{
  ...
  SEXP *args[] = {T, npaths, m0, lambda, phi, 
                  a0, a1, b1, gamma, 
                  h0, xmin1, z};

  char *names[] = {"T", "npaths", "m0", "lambda", "phi",
                   "a0", "a1", "b1", "gamma",
                   "h0", "xmin1", "z"};
                 ...
                 SEXP z)
{
  ...
  char *names[] = {"x", "h", "T", "npaths", "m0", "lambda",
                   "phi", "a0", "a1", "b1", "gamma",
                   "h0", "xmin1", "z"};

  sim_ar1ng11_chkargs(&T, &npaths, &m0, &lambda, &phi,
                      &a0, &a1, &b1, &gamma, &h0,
                      &xmin1, &z, &numprot);
