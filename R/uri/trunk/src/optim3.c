...
static void optimGa3_chk(SEXP *initPar, SEXP *parType, 
                         const int parLen, const int basePopSize, int *numprot)
{
...

{
  ...
  ok = isMatrix(pop) && basePopSize > 0 && 
    ncols(pop) >= 2*basePopSize && length(parType) == parLen &&
    IS_CHARACTER(parType);
...
static void getNextGen3(optimFun f, SEXP controlPar, SEXP pop,
                        SEXP parType, SEXP fargs, SEXP fvals, SEXP ix,
...
SEXP optimGa3(optimFun f, SEXP initPar, SEXP controlPar, SEXP parType,
              ...
              const long stopLag, const long minit, const long maxit, 
              const int minimize, const double tol, const int relTol)

  /*******************************************************************
   *
   *  Description: Uses genetic algorithms to optimize function x |-> f(x, controlPar).
   *
   *******************************************************************/

{
  ...
  for (gen = 0; gen < maxit && notConvergedYet; gen++) {
    ...
    getNextGen3(f, controlPar, pop, parType, fargs, 
...
SEXP testOptim3(SEXP initPar, SEXP parType, SEXP controlPar, 
                SEXP parLen, SEXP basePopSize, 
                SEXP stopLag, SEXP minit, SEXP maxit,
                SEXP minimize, SEXP tol, SEXP relTol, SEXP optimType)
{
  ...
  case 0:
    ...
    ans = optimGa3(testOptimFun, initPar, controlPar, parType,
