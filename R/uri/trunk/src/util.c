...
SEXP varRatio(SEXP x, SEXP maxOrd)
{
    ...
    for (ord = 2; ord <= ordMax; ord++) {
        ...
        for (i = ord; i < xLen; i++)
            returns[i] = REAL(x)[i] - REAL(x)[i - ord];
...
SEXP bvt_normuri(SEXP n, SEXP rho, SEXP jiggle)
{
    ...
    for (i = 0; i < m; i++) {
        for (j = 0; j < m; j++) {
            ...
            x2[i * m + j] = r * x1[i * m + j] + sqrt(1.0 - SQR(r)) * qnorm(u2, 0.0, 1.0, 1, 0);
