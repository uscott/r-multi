...
static void kalman1_chkargs(SEXP *y1,
                            SEXP *H1tr,
                            SEXP *F1,
                            SEXP *Q1,
                            SEXP *R1,
                            ...
                            int  *numprot)
{
  ...
  for (i = 0; i < N_SEXP_ARGS; i++)
    if (!isMatrix(*args[i]))
      error ("%s is not a matrix", names[i]);


  ok = 
    r == nrows(*F1) && r == ncols(*F1)   && n == nrows(*H1tr) &&
    r == nrows(*Q1) && r == ncols(*Q1)   && n == nrows(*R1)   &&
    n == ncols(*R1) && n == nrows(*A1tr) && k == ncols(*A1tr);

  if (!ok)
    error ("Some argument does not have the proper dimension");
...
static void kalman_recursion(carray y, 
                             carray Htr, 
                             carray F, 
                             carray Q, 
                             carray R,
...

{
  ...
  for (t = 0; t < T - 1; t++) {
    ...
    mp[t+1] = c + phi * (mp[t] + (P + sigma * rho * sqrt(hp[t])) / f * nu[t]);

    P = SQR(phi) * P 
      - SQR(phi) * SQR(P + sigma * rho * sqrt(hp[t])) / f 
      + SQR(sigma);
