...
static void makeCurve_chk(SEXP *xlist, SEXP *t, SEXP *mthNum, SEXPTYPE **types, int *numprot)
{
  long i, n, listlen;
  SEXP tmp;
  ...
  *types = (SEXPTYPE *) R_alloc(listlen, sizeof(SEXPTYPE));
  ...
  for (i = 1; i < length(*xlist); i++)
    if (n != length(GET_ELT(*xlist,i)))
      error ("mismatched lengths");

  if (n != length(*t) || n != length(*mthNum))
    error ("mismatched lengths");
...
#define MAX_CRV_MTHS 36
    ...
    if( !( nrows = length( tUnique ))) 
        return R_NilValue;
    if( !( ncols = imax(INTEGER(mthNum), tlen))) 
        return R_NilValue;
    ...
    for( i = 0; i < listlen; ++i )
        PROT2( xtmp[ i ] = NEW_STRING( ncols ), numprot );
    ...
    for( i = 0; i < nrows; ++i ) 
    {
        ...
        for( h = 0; h < listlen; ++h )
            for( j = 0; j < ncols; ++j )
                SET_ELT( xtmp[ h ], j, mkChar( "NA" ));

        for( k = 0; k < tlen; k++)
            if (INTEGER(t)[k] == t0 && (coli = INTEGER(mthNum)[k] - 1) < ncols)
                for (h = 0; h < listlen; h++)
                    SET_ELT(xtmp[h], coli, duplicate(STRING_ELT(GET_ELT(xlist,h), k)));

        for (h = 0; h < listlen; h++)
            for (j = 0; j < ncols; j++)
                SET_ELT(GET_ELT(xCrv,h), i + j * nrows, STRING_ELT(xtmp[h], j));
...
SEXP makeCurve2(SEXP xlist, SEXP dates, SEXP expDates)
{
  ...
  if (imin(INTEGER(mthNum), tlen) <= 0) error ("month numbers must be positive");
