...
/* Functions for dynamically allocating arrays
   ...
   Last modified by Uriel Scott Jul 02, 2003.
...
{
  if(!condition)
    error("assert2 failed in carray.c");
}
...
static int gaussj2(double **a, int n, double **b, int m)
  ...
  /*******************************************************************
   *
   *  Description: Gauss-Jordan Elimination adapted from NR in C.
   *    The arguments a, b are destroyed with the matrix double **a
   *    being replaced by its inverse.
   *
   *******************************************************************/

{
  int   *ixc,   *ixr,   *ipiv, *ixc2, 
        *ixr2,  *ipiv2, *ipiv3;
  ...
  if (invertible)
    for (l = 0; l < n; l++, ixr2--, ixc2--) {
      if (*ixr2 != *ixc2 && (a2 = a)) { /* Note assignment */
        for (k = 0; k < n; k++, a2++)
          SWAP((*a2)[*ixr2], (*a2)[*ixc2]);
      }
    }
     ...
     */
{
  ...
  /* is index in range? */
  if (index < 0 || index >= *DIM(a))
    error("Index out of range in subarray");
...
void transpose_matrix(carray mat, carray ans)
{
  ...
  for(i = 0; i < NROW(ans); i++)
    for (j = 0; j < NCOL(ans); j++)
      ARRAY2(tmp)[i][j] = ARRAY2(mat)[j][i];
...
void matrix_prod_n(carray *mat, int *trans, int n, carray ans)
{
  ...
  if (n <= 0) error ("Bad value for number of matrices in matrix_prod_n");
  else if (1 == n)
    if(*trans) transpose_matrix(*mat, ans);
    else copy_array(*mat, ans);
  else {

    tmp1 = make_zero_matrix(*trans ? NCOL(*mat) : NROW(*mat),
			    *trans ? NROW(*mat) : NCOL(*mat));

    if (*trans) transpose_matrix(*mat++, tmp1);
    else copy_array(*mat++, tmp1);
    ++trans;

    while (--n > 0) {
      tmp2 = make_zero_matrix(NROW(tmp1), *trans ? NROW(*mat) : NCOL(*mat));
...

{
  ...
  if (2 != DIM_LENGTH(a) || 2 != DIM_LENGTH(ai) ||
      2 != NROW(a) || 2 != NCOL(a) || 2 != NROW(ai) ||
      2 != NCOL(ai))
...
void kronecker(carray a, carray b, carray ans)
{
  ...
  for (i = 0; i < n; i++)
    for (j = 0; j < q; j++)
      for (k = 0; k < m; k++)
	for (h = 0; h < p; h++)
	  ARRAY2(ans)[h + p * k][j + q * i] = ARRAY2(a)[k][i] *
	    ARRAY2(b)[h][j];
     ...
     to be reversed in the transformation. */
{
  ...
  if (2 < DIM_LENGTH(A)) 
    error("Too many dimensions in carray_to_sexp");
