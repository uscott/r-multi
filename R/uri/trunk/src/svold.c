...
{
  int i;
  char *names[STATE_LEN] = {"alpha","delta","sigv",
                            "m","M","N",
                            "lambda", "h", "r"};
  SEXP args[STATE_LEN];
  ...
  if (length(getListElt(*currentState,"h")) != length(getListElt(*currentState,"r")))
...
SEXP hUpdate1(SEXP currentState, SEXP maxit)
{
  ...
  for (i = 1; i < T-1; i++) {
    ...
    c /= dgamma(1/xMode, /* shape = */ phi + 2, /* scale = */ 1/theta, 0);
  ...
  REAL(hNew)[0]   = exp(alpha + delta * log(REAL(hNew)[1])   + sigv * norm_rand());
  REAL(hNew)[T-1] = exp(alpha + delta * log(REAL(hNew)[T-2]) + sigv * norm_rand());
...
static void updateParams0(long T, double *alpha, double *delta,
...
SEXP algo2(SEXP currentState, SEXP info)
{
  ...
  PROT2(hOut = output_h ? allocMatrix(REALSXP, T - 2, n) : R_NilValue, numprot);
...
static SEXP getInitState(SEXP r, SEXP initState, int diffusePrior) 
{
  ...
  if ( !isNull(initState) ) {
    ...
    if (!initStateGiven)
      warning("some initial given value was null, using uninformed priors");
  ...
  if ( initStateGiven ) {
    ...
    /* Simulate log(h) */
    loghPtr[0] = rnorm(/*mu=*/ alpha/(1-delta), /*sigma=*/ sigv/sqrt(1-delta*delta));

    for (t = 1; t < T; t++)
      loghPtr[t] = alpha + delta * loghPtr[t-1] + sigv * norm_rand();

    for (t = 0; t < T; t++)
      hPtr[t] = exp(loghPtr[t]);
