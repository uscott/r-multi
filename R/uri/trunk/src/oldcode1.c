...
static void oneStepGrdSprd_old(long N, double S10, double S20, 
                               double sig1, double sig2, 
                               double rho, double r, double q1, 
                               ...
                               double *value, double h)
{
  ...
  A = (r*(1.0-rho*sig2/sig1)-(q2-q1*rho*sig2/sig1)+0.5*rho*sig2*(sig1-rho*sig2))*T;
  ...
  *value *= exp(m+0.5*sig1*sig1*T);
  ...
  term1 = a_0*NDist(-N/omega-rho*sig2*sqrt(T))*exp(rho*sig2*m/sig1+0.5*rho*rho*sig2*sig2*T);
  term2 = b_0*NDist(-N/omega);
  *value += term1 + term2;

  term1 = a_N*(1.0-NDist(N/omega-rho*sig2*sqrt(T)))*exp(rho*sig2*m/sig1+0.5*rho*rho*sig2*sig2*T);
...
void delta_to_strike(const int    *n,   const double *T, 
                     const double *S,   const double *vol, 
                     const double *del, const char  **opt, 
                     const double *r,   double       *q, 
                     double       *ans)
{
  ...
  for ( ; del < end; vol++, S++, T++, del++, ans++, opt++) {
    ...
    case 'c':
      ...
      t1   = -*vol * sqrt(tau) * qnorm(exp(*q * tau) * (*del),0, 1, 1, 0);
    ...
    case 'p':
      ...
      t1   = -*vol * sqrt(tau) * qnorm(1.0 + exp(*q * tau) * (*del), 0, 1, 1, 0);
      t2   = (*r - *q + 0.5 * SQR(*vol)) * tau;
      *ans = *S * exp(t1 + t2);
      break;
    ...
    case 's':
      ...
      t1   = -*vol * sqrt(tau) * qnorm(0.5 + 0.5 * exp(*q * tau) * (*del), 0, 1, 1, 0);
      t2   = (*r - *q + 0.5 * SQR(*vol)) * tau;
      *ans = *S * exp(t1 + t2);
      break;
                    ...
                    SEXP isRelUndCosts, SEXP isRelOptCosts)
{
  ...
  long T_len, S_len, i;
  SEXP strikesTmp, ans, tmp, undPL, strdPL, undTC, strdTC, tmp2,
    maxUndPosn, minUndPosn, maxStrdPosn, minStrdPosn,
    maxDelta, minDelta, maxVega, minVega, r0, q0;

  char *names[ANS_LEN] = {"undPL", "strdPL", "undTC", "strdTC",
                          "maxUndPosn", "minUndPosn", "maxStrdPosn", "minStrdPosn",
                          "maxDelta", "minDelta", "maxVega", "minVega"};
  ...
  for (i = 0; i < T_len; i++) {
    ...
    tmp = dvHedge(posn, strikesTmp,
                  optionTypes, hedgePeriod,
    ...
    REAL(maxUndPosn)[i] = double_max(REAL(getListElt(tmp, "undPosn")), S_len - i);
    REAL(minUndPosn)[i] = double_min(REAL(getListElt(tmp, "undPosn")), S_len - i);
    REAL(maxStrdPosn)[i] = double_max(REAL(getListElt(tmp, "strdPosn")), T_len - i);
    REAL(minStrdPosn)[i] = double_min(REAL(getListElt(tmp, "strdPosn")), T_len - i);   

    REAL(maxDelta  )[i] = double_max(REAL(getListElt(tmp, "deltas" )), S_len - i);
    REAL(minDelta  )[i] = double_min(REAL(getListElt(tmp, "deltas" )), S_len - i);
    REAL(maxVega    )[i] = double_max(REAL(getListElt(tmp, "vegas"   )), T_len - i);
    REAL(minVega    )[i] = double_min(REAL(getListElt(tmp, "vegas"   )), T_len - i);   
...
static void optimGa3_chk(SEXP *initPar, const int parLen, const int basePopSize, int *numprot)
{
  ...
  if (isMatrix(*initPar)) {
    ...
    for (j = 0; j < MIN(ncols(*initPar), basePopSize); j++)
      memcpy(REAL(GET_ELT(tmp, j)), matcol1(*initPar, j), parLen * sizeof(double));
...

{
  ...
  if (!ok)
    error ("bad argument passed to mutate2");
  ...
  for (j = 0; j < basePopSize; j++) {
    ...
    for (i = 0; i < parLen; i++, ptr1++, ptr2++)
      *ptr2 = ISNA(*ptr1) ? norm_rand() : *ptr1 + norm_rand() * ABS(*ptr1);
...

{
  ...
  ok = isNewList(pop) && parLen &&
    bigPopSize == BIG_POP_SIZE(basePopSize);

  if (!ok)
    error ("bad arguments in breed3");
...
static void getNextGen3(optimFun f, SEXP controlPar, SEXP pop,
                        SEXP fvals, SEXP ix, SEXP tmp, 
                        const int basePopSize, const int minimize)
{
  ...
  for (j = 0; j < basePopSize; j++, ixPtr++) {
    ...
    memcpy(REAL(GET_ELT(tmp,j)),REAL(GET_ELT(pop,*ixPtr)),parLen*sizeof(double));

  }

  for (j = 0; j < basePopSize; j++) {

    memcpy(REAL(GET_ELT(pop,j)), REAL(GET_ELT(tmp,j)),parLen*sizeof(double));
...
#define ANS_LEN 3
SEXP optimGa3(optimFun f, SEXP initPar, SEXP controlPar,
              const int parLen, const int basePopSize, const long maxit, 
...

{
  ...
  for (j = 0; j < basePopSize; j++) {
      ...
      memcpy(REAL(GET_ELT(pop,j)),REAL(GET_ELT(initPar,j)),parLen*sizeof(double));
  ...
  for (j = 0; j < numAnsCols; j++)
    memcpy(matcol1(popMatrix, j), REAL(GET_ELT(pop,j)), parLen*sizeof(double));
...
static void ng11_w1(const long T, const double *x, const double *par,
                    double w0, int llonly, 
...
static double ll_ngarch11_w1(SEXP par, SEXP x)
{
  ...
  ng11_w1(length(x), REAL(x), REAL(par), 
          REAL(par)[WEIGHT_INDEX],
          /* llonly = */ 1, 
          /* h = */ (double *)NULL, 
          /* z = */ (double *)NULL, 
          &ll);
...
static void adjustInitPar_w1(SEXP *initPar, int parLen, int *numprot)
{
  ...
  if (isNewList(*initPar)) {
    ...
    if (!R_FINITE(REAL(par)[WEIGHT_INDEX]))
      REAL(par)[WEIGHT_INDEX] = 1;
...
#define ANS_LEN 7
SEXP fit_ngarch11_w1(SEXP x, SEXP initPar, SEXP fitInit, 
                     ...
                     SEXP stopLags, SEXP minit, SEXP maxit, SEXP options)
{
  ...
  /* Do the fitting! */
  fit1 = optimGa2(ll_ngarch11_w1, initPar, x, parLen, 
  ...
  if (gradToo) {
    ...
    fit2 = optimGradient2(ll_ngarch11_w1, getListElt(fit1,"par"), x, dtol, 0, 0, gradMaxIt);
    PROT2(fit2, numprot);
