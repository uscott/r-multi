...
SEXP fcm_EuroCall0(SEXP tau, SEXP price, SEXP strike, SEXP vol, SEXP nGrdPts, SEXP nTimeSteps)
{
    ...
    for (n = 0; n < N; n++) {
        ...
        for (j = 0; j < 2 * M + 1; j++) {
            ...
            for (i = 0; i < 2 * M; i++) {
                ...
                V[j] += a[i] * x * (NORM_DIST((i-j+1)/w - sig*sqrt(dt)) - NORM_DIST((i-j)/w - sig*sqrt(dt)));
                V[j] += b[i] * (NORM_DIST((i-j+1)/w) - NORM_DIST((i-j)/w));
