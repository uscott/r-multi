...
#define IS_LEAP( year ) (0==(year)%4 && (0!=(year)%100 || 0==(year)%400))
...
{
    return 
        __mon_yday[ IS_LEAP( tm->year )][ tm->month ] + ( tm->day - 1 );
}

#define ERRIF(x) if ( x ) return 1;
