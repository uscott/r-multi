...
#define DIGAMMA( x, phi, theta )  (dgamma(1.0/(x), (phi) + 2.0, 1.0/(theta), 0))
...
(R_pow(h,-1.5) * exp(-0.5*SQR(r)/h) * exp(-0.5 * SQR(log(h) - mu) / SQR(sigma)))
...
SEXP algo3(SEXP r, SEXP info, SEXP initState)
{
    ...
    for (n = -initLoops; n < numLoops; n++) 
    {
        ...
        */
        ...
        matrix_prod(tmp_2x2, XtX, 0, 0, tmp_2x2); /* tmp_2x2 <- Mi * Mnew * X'X */
