...
static double Inner(double S, double S1, double S2, double sig1, double sig2, double rho, 
                    double r, double q1, double q2, double tau, double X)
{
    ...
    if (S + X > 0) {
        ...
        if (1 == rho*rho)
            return (M2 > log(S+X) ? exp(A)*S2*pow(S/S1, rho*sig2/sig1)-(S+X) : 0);
        else {
            x1 = (M2+sig*sig-log(S+X))/sig;
            x2 = x1-sig;
            return exp(A)*S2*pow(S/S1, rho*sig2/sig1)*NDist(x1)-(S+X)*NDist(x2);
        }
    }
    else
...
static double OneStepGridSpread(long nGrdPts, double S1, double S2, 
                                ...
                                double q2, double X, double T)
{
    ...
    const double A = T * (r*(1.0 - c / sig1) - (q2 - q1 * c / sig1) + 0.5 * rho * sig2 * (sig1 - c));
    ...
    if (0.0 >= T)
        return 0.0 > T ? NA_REAL : MAX(S2 - S1, 0.0);
    ...
    b0 = Inner(x1,S1,S2,sig1,sig2,rho,r,q1,q2,T,X) - a0 * pow(x1, c / sig1);

    term1 = a0 * NORM_DIST(-N/w - c * sqrt(T)) * exp(c * m / sig1 + 0.5 * c * c * T);
    term2 = b0 * NORM_DIST(-N/w);
    term3 = aN * (1.0 - NORM_DIST(N/w - c*sqrt(T))) * exp(c * m / sig1 + 0.5 * c * c *T);
...
static void margrabeApprox(double tau, double S1, double S2, double K, 
                           ...
                           char optionType, int calcDeltas,
                           double *valG)
{
    /* 
    ...
    *valG = NA_REAL;
    if (calcDeltas)
        valG[PEARSON0_PRICE_INDEX] = valG[PEARSON0_D1_INDEX] = valG[PEARSON0_D2_INDEX] = NA_REAL;
...
#undef OLD_CODE_BELOW
    ...
    if (!valG)
        error ("null pointer passed to margrabeApprox");

    if (calcDeltas)
        valG[PEARSON0_D1_INDEX] = valG[PEARSON0_D2_INDEX] = 0;
    ...
    if (K > 0) {
        ...
        rho2p = -(vol3p/vol3)*rho2 + (cp/c-dp/d)/(vol2*vol3*tau);

        margrabe0(tau, S3, S2, vol3, vol2, rho2, r, q1, q2, optionType, tmp);

        if (calcDeltas)
            valG[PEARSON0_D1_INDEX] += tmp[MARGRABE0_V1_INDEX]*vol3p + tmp[MARGRABE0_ETA_INDEX]*rho2p;
        ...
        if (calcDeltas)
            valG[PEARSON0_D2_INDEX] += tmp[MARGRABE0_V2_INDEX]*vol3p + tmp[MARGRABE0_ETA_INDEX]*rho2p;
              ...
              double *valG)
{
    ...
    if (0.0 == K || ABS(rho) > 1 || nGrdPts <= 0 || !R_FINITE((double)nGrdPts)) {
        ...
        margrabeApprox(tau, S1, S2, K, vol1, vol2, rho, r, q1, q2, optionType, calcDeltas, valG);
        return;

    }

    *valG = OneStepGridSpread(nGrdPts, S1, S2, vol1, vol2, rho, r, q1, q2, K, tau);

    if (calcDeltas) {

        valUp = OneStepGridSpread(nGrdPts, (1+u)*S1, S2, vol1, vol2, rho, r, q1, q2, K, tau);
        valDn = OneStepGridSpread(nGrdPts, (1-u)*S1, S2, vol1, vol2, rho, r, q1, q2, K, tau);

        valG[PEARSON0_D1_INDEX] = (valUp-valDn)/(2*u*S1);

        valUp = OneStepGridSpread(nGrdPts, S1, (1+u)*S2, vol1, vol2, rho, r, q1, q2, K, tau);
        valDn = OneStepGridSpread(nGrdPts, S1, (1-u)*S2, vol1, vol2, rho, r, q1, q2, K, tau);
    ...
    case 'c':
        break;
    case 'p':
        valG[PEARSON0_PRICE_INDEX] -= S2*exp(-q2*tau)-S1*exp(-q1*tau)-K*exp(-r*tau);
        if (calcDeltas) {
    ...
    case 's':
        ...
        valG[PEARSON0_PRICE_INDEX] -= (S2*exp(-q2*tau)-S1*exp(-q1*tau)-K*exp(-r*tau));
        if (calcDeltas) {
            valG[PEARSON0_D1_INDEX] = 2 * valG[PEARSON0_D1_INDEX] + exp(-q1*tau);
            valG[PEARSON0_D2_INDEX] = 2 * valG[PEARSON0_D2_INDEX] - exp(-q2*tau);
        }
        break;
    default:
        valG[PEARSON0_PRICE_INDEX] = NA_REAL;
...
static void pearson_chkargs(SEXP *tau, SEXP *S1, SEXP *S2, 
...
SEXP pearson(SEXP tau, SEXP S1, SEXP S2, SEXP K,
             SEXP vol1, SEXP vol2, SEXP rho, SEXP r, 
             SEXP q1, SEXP q2, SEXP opt, SEXP calcDeltas, 
             SEXP grdPts)
{
    int numprot = 0;
    ...
    pearson_chkargs(&tau, &S1, &S2, &K, &vol1, &vol2, 
                    &rho, &r, &q1, &q2, &opt,
                    &numprot);
    ...
    for (i = 0; i < N; i++) {
        ...
        pearson0(REAL(tau)[i] / 365.0, REAL(S1)[i], REAL(S2)[i],
                 REAL(K)[i], REAL(vol1)[i], REAL(vol2)[i],
                 REAL(rho)[i], REAL(r)[i], REAL(q1)[i],
                 REAL(q2)[i], *CHAR(GET_ELT(opt, i)), nGrdPts, 
...
#define NARGS 11
static void listPearson_chk(SEXP *tau, SEXP *S1, SEXP *S2, SEXP *K, 
                            SEXP *vol1, SEXP *vol2, SEXP *rho, SEXP *r, 
...
SEXP listPearson(SEXP tau, SEXP S1, SEXP S2, SEXP K,
                 SEXP vol1, SEXP vol2, SEXP rho, SEXP r, 
                 SEXP q1, SEXP q2, SEXP opt, SEXP calcDeltas, 
                 SEXP grdPts)
{
    ...
    listPearson_chk(&tau, &S1, &S2, &K, &vol1, &vol2, &rho,
                    &r, &q1, &q2, &opt, &numprot);
